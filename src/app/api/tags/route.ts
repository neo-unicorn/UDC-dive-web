...
export async function GET(request: NextRequest) {
    try {
        ...
        type TagWithCount = { slug: string; nameZh: string; nameEn: string; _count: { articles: number } };
        return NextResponse.json({ tags: tags.filter((t: TagWithCount) => t._count.articles > 0).map((t: TagWithCount) => ({ slug: t.slug, name: locale === 'zh' ? t.nameZh : t.nameEn, count: t._count.articles })) });
