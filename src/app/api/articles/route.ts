...
export async function GET(request: NextRequest): Promise<NextResponse<ArticleListResponse | ErrorResponse>> {
    try {
        ...
        return NextResponse.json({ articles: articles.map((a: { id: string; slug: string; locale: string; title: string; excerpt: string | null; coverImage: string | null; publishedAt: Date | null; viewCount: number; category: { slug: string; nameZh: string; nameEn: string } | null; tags: { tag: { slug: string; nameZh: string; nameEn: string } }[] }) => ({ id: a.id, slug: a.slug, locale: a.locale, title: a.title, excerpt: a.excerpt, coverImage: a.coverImage, publishedAt: a.publishedAt?.toISOString() || null, viewCount: a.viewCount, category: a.category ? { slug: a.category.slug, name: a.locale === 'zh' ? a.category.nameZh : a.category.nameEn } : null, tags: a.tags.map((t: { tag: { slug: string; nameZh: string; nameEn: string } }) => ({ slug: t.tag.slug, name: a.locale === 'zh' ? t.tag.nameZh : t.tag.nameEn })) })), pagination: { page, limit, total, pages: Math.ceil(total / limit) } });
