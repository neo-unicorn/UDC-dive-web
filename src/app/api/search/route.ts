...
export async function GET(request: NextRequest) {
    try {
        ...
        return NextResponse.json({ query, results: articles.map(a => ({ id: a.id, slug: a.slug, locale: a.locale, title: a.title, excerpt: a.excerpt, coverImage: a.coverImage, publishedAt: a.publishedAt?.toISOString(), category: a.category ? { slug: a.category.slug, name: a.locale === 'zh' ? a.category.nameZh : a.category.nameEn } : null, highlight: a.excerpt || '' })), pagination: { page, limit, total, pages: Math.ceil(total / limit) } });
